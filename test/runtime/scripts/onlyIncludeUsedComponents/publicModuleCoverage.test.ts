 ...
 *
 * On what counts as a public module: `package.json` `exports` is generated by
 * `scripts/build/list-exports.ts` (run by `pnpm build`) from the files `tsc -p src` emits
 * into `dist/`, so the published subpaths are exactly the emitted modules. The enumeration
 * below therefore replays that emission: the entries of `src/`, minus what `src/tsconfig.json`
 * excludes (read from the file, not hardcoded, so a future exclude stays in sync) and minus
 * the files tsc does not emit a module for.
 */
describe("REACT_DSFR_MODULE_TO_DSFR_COMPONENTS exhaustiveness", () => {
    ...
    it("public subpaths are reachable through the package.json exports map", () => {
        // The exports map is generated by scripts/build/list-exports.ts from the files
        // actually emitted in `dist/`. Every enumerated src/ subpath must therefore have an
        // explicit export of its own — a `./*` wildcard would silently let the map drift
        // from the emitted files (and cannot resolve directories / extension-less targets
        // anyway). The dedicated packaging test (test/runtime/package/package-exports.test.ts)
        // regenerates the map and verifies every target exists; here we only guard that the
        // enumeration contract is covered and that the isolation of the Discord entry holds.
        ...
        // No wildcard: the map is an explicit, exhaustive list.
        expect(exports["./*"]).toBeUndefined();

        for (const subpath of getPublicSubpaths()) {
            expect(
                exports[`./${subpath}`],
                `src/${subpath} must have an explicit package.json export`
            ).toBeDefined();
        }
        ...
        // Deep subpaths of the reported consumer errors are officially public.
        for (const subpath of [
            ...
            expect(
                exports[subpath],
                `${subpath} must have an explicit package.json export`
            ).toBeDefined();
        }
    });
});
